...
# AIDL/HIDL corpus, and the binderdump column profile. Run with no arguments from a
# repo checkout, or point --so/--corpus/--profile at downloaded release artifacts.
set -euo pipefail

usage() {
    cat <<'USAGE'
Usage: install_dissector.sh [--so PATH] [--corpus PATH] [--profile DIR]
...
binderdump-aidl/data). To install from release downloads, pass:
  ...
  --so PATH        the libbinderdump-<tag>-x86_64-linux-gnu.so (or a built .so)
