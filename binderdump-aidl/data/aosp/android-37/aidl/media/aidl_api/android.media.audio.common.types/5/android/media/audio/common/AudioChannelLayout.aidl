...
union AudioChannelLayout {
  ...
  int voiceMask;
  int acnMask;
  const int ACN_CHANNEL_COUNT_BIT_MASK = ((1 << 8) - 1) /* 255 */;
  const int ACN_SOURCE_LAYOUT_BIT_SHIFT = 8;
  const int ACN_SOURCE_LAYOUT_BIT_MASK = (((1 << 4) - 1) << ACN_SOURCE_LAYOUT_BIT_SHIFT) /* 3840 */;
  ...
  parcelable Ambisonics {
    ...
    const int MIN_CHANNEL_COUNT = FULL_SPHERE_CHANNEL_COUNT_ORDER_0 /* 1 */;
    const int MAX_CHANNEL_COUNT = FULL_SPHERE_CHANNEL_COUNT_ORDER_14 /* 225 */;
    enum SourceLayout {
      FULL_SPHERE = 0,
      HORIZONTAL = 1,
    }
